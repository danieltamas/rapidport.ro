// POST /api/admin/users/[id]/block — admin-imposed block (separate from
// GDPR delete). Sets users.blockedAt = now() and users.blockedReason.
// While blocked, getUserSession returns null for this user.
//
...
export default defineEventHandler(async (event) => {
  ...
  if (row.blockedAt) {
    throw createError({ statusCode: 409, statusMessage: 'already_blocked' });
  }
  ...
  await db.transaction(async (tx) => {
    await tx
      .update(users)
      .set({ blockedAt: new Date(), blockedReason: body.reason })
      .where(eq(users.id, id));
